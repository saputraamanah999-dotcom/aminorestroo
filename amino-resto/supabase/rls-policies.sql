...
create policy "profiles admin insert" on public.profiles for insert with check (auth.uid() = id or public.is_admin());
...
create policy "public select visible reviews" on public.reviews for select using (is_visible = true or public.is_admin() or auth.uid() = user_id);
create policy "users insert own reviews" on public.reviews for insert to authenticated with check (auth.uid() = user_id);
create policy "users update own unreplied reviews" on public.reviews for update to authenticated using (auth.uid() = user_id and coalesce(reply, admin_reply) is null) with check (auth.uid() = user_id and coalesce(reply, admin_reply) is null);
create policy "users delete own unreplied reviews" on public.reviews for delete to authenticated using (auth.uid() = user_id and coalesce(reply, admin_reply) is null);
