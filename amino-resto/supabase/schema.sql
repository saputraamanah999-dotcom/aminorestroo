...
create table public.menu_items (id uuid primary key default gen_random_uuid(), category_id uuid references public.categories(id) on delete set null, name text not null, description text, price integer not null default 0, image_url text, dietary_label text, is_available boolean default true, is_featured boolean default false, is_recommended boolean default false, is_best_seller boolean default false, is_new boolean default false, sort_order int default 0, created_at timestamptz default now(), updated_at timestamptz default now());
...
create table public.reviews (id uuid primary key default gen_random_uuid(), user_id uuid references public.profiles(id) on delete set null, menu_item_id uuid references public.menu_items(id) on delete set null, order_id uuid references public.orders(id) on delete set null, rating int not null check(rating between 1 and 5), comment text not null, reply text, admin_reply text, reply_by uuid references public.profiles(id) on delete set null, reply_at timestamptz, is_verified boolean default false, is_visible boolean default true, created_at timestamptz default now(), updated_at timestamptz default now());
create table public.restaurant_settings (id uuid primary key default gen_random_uuid(), restaurant_name text default 'Amino Resto Bali', address text default 'Uluwatu St No.77, Ungasan, Bali', announcement text default 'Welcome to Amino Resto. PRICES ARE NOT FIXED. Tax 10% and service 5% apply.', opening_hours text default 'Open daily 08:00 - 22:00', breakfast_hours text default '08:00 - 11:00', lunch_dinner_hours text default '12:00 - 22:00', open_status boolean default true, rating_average numeric default 4.9, whatsapp text, maps_url text, maps_embed_url text, gojek_url text, grabfood_url text, logo_url text, hero_image_url text, qris_image_url text, tax_rate numeric default 0.10, service_rate numeric default 0.05, primary_color text default '#0f5b55', accent_color text default '#c9a66b', email_notifications boolean default true, created_at timestamptz default now(), updated_at timestamptz default now());
create table public.gallery (id uuid primary key default gen_random_uuid(), title text, category text, image_url text, is_visible boolean default true, sort_order int default 0, created_at timestamptz default now(), updated_at timestamptz default now());
